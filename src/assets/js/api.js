...
export const Api = {
  ...
  updateCarritoItem: async (detalleId, cantidad) => {
    const { data } = await apiClient.put(`/carrito/detalle/${detalleId}`, {
      cantidad,
    });
    return data;
  },
  ...
  cerrarCarrito: async (usuarioId, datosTransaccion) => {
    const { data } = await apiClient.post(`/carrito/cerrar/${usuarioId}`, datosTransaccion);
    return data;
  },
  ...
  marcarComoEnviado: async (carritoId) => {
    const { data } = await apiClient.put(`/carrito/marcar-enviado/${carritoId}`);
    return data;
  },

  // ===== MENSAJES DE CONTACTO =====
  enviarMensaje: async (mensajeData) => {
    const { data } = await apiClient.post("/mensajes", mensajeData);
    return data;
  },

  getMensajes: async () => {
    const { data } = await apiClient.get("/mensajes");
    return data;
  },

  cambiarEstadoMensaje: async (id, estado) => {
    const { data } = await apiClient.put(`/mensajes/${id}/estado`, { estado });
    return data;
  },
