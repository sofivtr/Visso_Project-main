...
// Precios de tratamientos adicionales
const PRECIOS_TRATAMIENTOS = {
  antirreflejo: 15000,
  filtroAzul: 20000,
  fotocromático: 25000, // Diferencia entre Blanco y Fotocromático
};

function calcularPrecioCotizacion(precioBase, cotizacionData) {
  let total = precioBase;
  
  // Agregar costo de tratamientos
  ...
  // Agregar costo si es fotocromático
...
function Productos() {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(p => {
      // Filtrar por categoría (comparar con categoria.nombre del backend)
  ...
  const openModal = (p) => {
    setSelected(p);
    setQty(1);
    // Mostrar formulario de cotización solo si es categoría ÓPTICA
  ...
  const addToCartFromModal = async () => {
    ...
    try {
      ...
      // Si es producto óptico y tiene cotización, crear la cotización primero
      if (showCotizacion) {
        ...
        // Calcular precio total con tratamientos
        ...
        const cotizacion = await Api.createCotizacion({
          ...
          valorAprox: precioTotal, // Precio total calculado
        });
      ...
      // También actualizar el carrito local para que se muestre el contador
      cartAdd({
        id: selected.id,
        ...
        <div className="modal-dialog modal-xl modal-dialog-centered">
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              {/* FORMULARIO DE COTIZACIÓN - Solo para productos ÓPTICA */}
                  {showCotizacion && (
